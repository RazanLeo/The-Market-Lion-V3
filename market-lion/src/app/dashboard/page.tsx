...
export default function DashboardPage() {
  ...
  const { analysis, levels, sizing, asset, coreRows, schoolRows, indRows, direction } = useMemo(() => {
    try {
      ...
      return { analysis, levels, sizing, asset, coreRows, schoolRows, indRows, direction };
    } catch (e) {
      console.error("dashboard compute error", e);
      // Fallback empty state
      return { analysis: null, levels: null, sizing: null, asset: ASSETS[0], coreRows: [], schoolRows: [], indRows: [], direction: "NEUTRAL" as const };
    }
  }, [options]);
  ...
  return (
    <>
      ...
      <main className="max-w-[1600px] mx-auto px-3 md:px-5 py-5 space-y-5">
        ...
        {analysis && (<>
          <TableShell number={3} title="جدول ٣ — التحليل الفني / الأدوات الرئيسية الأساسية" weight="30%" subtitle="23 أداة">
            <RowVoteTable rows={coreRows} weightLabel="30%" totalWeightPct={30}/>
          </TableShell>
          <TableShell number={4} title="جدول ٤ — جميع مدارس التحليل الفني" weight="25%" subtitle="48 مدرسة">
          ...
          {sizing && levels && (
            <Table8Plan asset={options.asset} lots={sizing.lots} levels={levels} capital={options.capital}
              riskPct={options.riskPct} riskAmount={sizing.riskAmount}
              mode={options.mode} tf={options.tf} direction={direction}/>
          )}
        </>)}
        <ChatPanel context={{
          asset: options.asset, tf: options.tf, mode: options.mode,
          riskPct: options.riskPct, capital: options.capital,
          confluence: analysis?.confluence, summaries: analysis?.tables,
        }}/>
      </main>
      <Footer/>
    </>
