...
export default function SignupPage() {
  ...
  const broker = BROKERS.find(x => x.id === brokerId) ?? BROKERS[0];
      ...
      <div className="w-full max-w-2xl gold-card p-8 shadow-gold">
        ...
        <form action="/api/auth/signup" method="post" className="grid sm:grid-cols-2 gap-4">
          ...
          <div className="sm:col-span-2 mt-2 border-t border-gold-500/20 pt-4">
            ...
            <p className="text-xs text-zinc-400 mb-3">{broker.description}</p>
          </div>
          ...
          {broker.fields.map(f => {
            if (f.type === "checkbox") {
              return (
                <label key={f.key} className="sm:col-span-2 flex items-center gap-3 cursor-pointer">
                  <input type="checkbox" name={`broker_${f.key}`} className="w-4 h-4 accent-gold-500"/>
...
function Field({ label, name, type = "text", placeholder, required }:
  ...
  return (
    <div>
      <label className="text-sm text-gold-400 mb-1 block">
